...
class SeoFrontend
{
    ...
    private function canonical()
    {
        if( ! $this->isNoindex() ){
            ...
            $canonical = apply_filters( 'wpc_seo_canonical', $canonical, $this );
    ...
    public function showSeoDescription()
    {
        if( $seoText = $this->get('seoDescription') ){
            ...
            if ( is_plugin_active( 'elementor/elementor.php' ) ) {
                $all_wp_hooks   = flrt_print_filters_for('the_content' );
                $hooksToDisable = [];
                    ...
                    foreach ( $all_wp_hooks->callbacks as $priority => $hooks ){
                        if( is_array( $hooks ) ){
                            foreach ($hooks as $function => $args ){
                                ...
                                if( strpos( $function, 'builder_in_content' ) !== false ){
                ...
                if( ! empty( $hooksToDisable ) ){
                    foreach ( $hooksToDisable as $hookName => $priority ){
                        remove_action('the_content', $hookName, $priority );
                    }
                }
            }
            ...
            if ( is_plugin_active( 'elementor/elementor.php' ) ) {
                if( ! empty( $hooksToDisable ) ){
                    $el = new \Elementor\Frontend;
                    $el->add_content_filter();
                }
            }
        }
    }
