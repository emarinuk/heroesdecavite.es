    ...
    function hook_newsletter_action($action, $user, $email) {
        ...
        if ($newsletter->antibot_form_check()) {
            ...
            foreach ($lists as $id => $value) {
                ...
                if (!$list || $list->status == TNP_List::STATUS_PRIVATE) {
                    if (current_user_can('administrator')) {
                        $newsletter->dienow('Shjown only to administrators. List change not allowed.', 'Please check if the list ' . $list->id . ' is marked as "private".', 400);
                    }
            ...
            if (empty($_request['r'])) {
                ...
                if (substr($url, 0, 1) === '/') {
                    // Relative URL, it's ok
                } else {
                    $home_host = parse_url(home_url(), PHP_URL_HOST);
                    $home_host = str_replace('www.', '', $home_host);
                    $url_host = parse_url($url, PHP_URL_HOST);
                    $url_host = str_replace('www.', '', $url_host);

                    if ($url_host !== $home_host) {
                        $logger->debug('URL not matching the site: "' . $url . '" - ' . home_url());
                        Newsletter::instance()->dienow('Invalid redirect.', 'Please check the redirect URL set on the newsletter, it should start with your site URL.', 400);
                    }
                }
            }
...
class TNP_Report {
    ...
    function update($benchmark = null) {
        if ($this->total > 0) {
            $this->open_rate = round($this->open_count / $this->total * 100, 1);
            $this->click_rate = round($this->click_count / $this->total * 100, 1);
            $this->error_rate = round($this->error_count / $this->total * 100, 1);
            if ($this->open_count > 0) {
                $this->reactivity = round($this->click_count / $this->open_count * 100, 1);
            }
        } else {
            $this->open_rate = 0;
            $this->click_rate = 0;
