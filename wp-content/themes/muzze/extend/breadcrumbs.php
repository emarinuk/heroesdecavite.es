<?php function muzze_breadcrumbs_header(){
    if( ! ( class_exists( 'woocommerce' ) && is_woocommerce() ) ){
        if( get_post_meta(  muzze_get_current_id() ,'muzze_met_show_breadcrumbs', true ) != 'no' ){
            muzze_breadcrumbs();
        }
    }else{
        $args = array(
	...
	$html = '<div id="breadcrumbs">';
				...
				$html .= '<ul class="breadcrumb">';
			        ...
			        if ( is_category() ) {
				        ...
				        $html .= wp_kses_post($before ) . esc_html__('Archive by date', 'muzze').' ' . get_the_time('d') . '"' . $after;

			        } elseif ( is_month() ) {

				        $html .= '<li><a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a></li>' . $separator . ' ';
				        $html .= wp_kses_post($before) . esc_html__('Archive by month', 'muzze').' ' . get_the_time('F') . '"' . $after;

			        } elseif ( is_year() ) {

			        	$html .= wp_kses_post($before) . esc_html__('Archive by year', 'muzze').' ' . get_the_time('Y') . '"' . $after;
			            ...
			            $html .= wp_kses_post( $before ) . esc_html__('Search results for', 'muzze').' ' . get_search_query() . '"' . $after;
			        	...
			        	$html .= wp_kses_post( $before ) . esc_html__('Archive by tag', 'muzze').' ' . single_tag_title('', false) . '"' . $after;
			        } elseif ( is_author() ) {
			        global $author;
			        $userdata = get_userdata($author);
			        	$html .= wp_kses_post( $before ) . esc_html__('Articles posted by', 'muzze').' ' . $userdata->display_name . '"' . $after;
			        } elseif ( is_home() ){
			        	$html .= wp_kses_post( $before ) . esc_html__('Blog', 'muzze').'&nbsp;' . $after;
			        }elseif ( is_404() ) {
			        	$html .= wp_kses_post( $before ) . esc_html__('You got it Error 404 not Found', 'muzze').'&nbsp;' . $after;
			        }
