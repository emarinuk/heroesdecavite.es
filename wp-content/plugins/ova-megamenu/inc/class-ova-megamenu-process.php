...
function ova_megamenu_add_custom_nav_fields( $menu_item ) {
    ...
    // $menu_item->menu_align = get_post_meta( $menu_item->ID, '_menu_item_menu_align', true );

    // Top
    // $menu_item->top_title = get_post_meta( $menu_item->ID, '_menu_item_top_title', true );
    // $menu_item->top_desc = get_post_meta( $menu_item->ID, '_menu_item_top_desc', true );
    // $menu_item->top_linkbottom = get_post_meta( $menu_item->ID, '_menu_item_top_linkbottom', true );
    // $menu_item->top_bg = get_post_meta( $menu_item->ID, '_menu_item_top_bg', true );
    ...
    // $menu_item->sub_img = get_post_meta( $menu_item->ID, '_menu_item_sub_img', true );
			...
			<dt class="menu-item-handle">
				...
				<span class="item-controls">
					...
					<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_html_e('Edit Menu Item','ova-megamenu'); ?>" href="<?php
						...
						?>"><?php esc_html_e('Edit Menu Item','ova-megamenu'); ?></a>
				</span>
			</dt>
			</dl>
			<div class="menu-item-settings ova-menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
				...
				<p class="description description-wide">
					<label for="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>">
						...
						<select id="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>"
							...
							<option value="one-column" <?php if(esc_attr($item->menu_column) == "one-column"){echo 'selected="selected"';} ?>><?php echo 'One Column'; ?></option>
							<option value="two-columns" <?php if(esc_attr($item->menu_column) == "two-columns"){echo 'selected="selected"';} ?>><?php echo 'Two Columns'; ?></option>
							<option value="three-columns" <?php if(esc_attr($item->menu_column) == "three-columns"){echo 'selected="selected"';} ?>><?php echo 'Three Columns'; ?></option>
							<option value="four-columns" <?php if(esc_attr($item->menu_column) == "four-columns"){echo 'selected="selected"';} ?>><?php echo 'Four Columns'; ?></option>
						</select>
					</label>
				</p>
				...
				<p class="description">
					<label for="edit-menu-item-linkhide-<?php echo esc_attr($item_id); ?>">
						<input type="checkbox" id="edit-menu-item-linkhide-<?php echo esc_attr($item_id); ?>" class="code edit-menu-item-custom" value="linkhide"
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($this->hideul){
				...
				if(!empty($this->column)){
					$output .= "\n$indent$out_div<ul class=\"ova-mega-menu ".esc_attr($this->column)."  dropdown-menu\"  role=\"menu\">\n";
				}else{
					$output .= "\n$indent$out_div<ul class=\"dropdown-menu submenu\" role=\"menu\">\n";
				}	
			}
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if(!$this->hideul){
				if(!(empty($item->linkhide))){
					...
					$item_output .= '<li class="title">'.esc_html($item->title).'</li>';
					...
					$item_output .= '<a'. $attributes .' class="col_heading">';
					...
					$item_output .= ( $args->has_children ) ? ' <i class="arrow_carrot-down force-elegant-icons"></i></a><button type="button" class="dropdown-toggle"><i class="arrow_carrot-down force-elegant-icons"></i></button>' : '</a>';
				...
				$item_output = $args->before;
				$item_output .= '<li><a'. $attributes.' class="' .$mega_item_active.'" >';
				$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
				
				// if(!empty($item->tip_label)){
				// 	$item_output .= esc_html($item->tip_label);
				// }

				$item_output .= ( $args->has_children ) ? ' <span class="fa fa-caret-down show_dropmenu"></span></a><button type="button" class="dropdown-toggle"><i class="arrow_carrot-down force-elegant-icons"></i></button>' : '</a></li>';
