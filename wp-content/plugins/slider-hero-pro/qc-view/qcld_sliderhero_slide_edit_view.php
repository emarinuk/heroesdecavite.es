...
function qcld_sliderhero_edit_slider_view( $_row, $_id, $_slider ) {
    ...
    <div class="qchero_slider_view_wrapper">
                    ...
                    foreach ( $_row as $rows ) {
                        ...
                        switch ( $rows->type ) {
                            ...
                            default: ?>
                                ...
                                <li id="qcheroitem_<?php echo $rows->id; ?>" data-sid="<?php echo $rows->id; ?>" class="qcheroitem <?php echo $draftclass; ?> ">
                                    <div class="qcheroitem-img-container">
										...
										<div class="qcheroitem-image">
											...
											<?php if(isset($rows->image_link) and $rows->image_link!=''): ?>
												<img data-slide-id="<?php echo $rows->id; ?>" src="<?php echo ($_slider[0]->type=='video'?QCLD_SLIDERHERO_IMAGES.'/video.png':$rows->image_link); ?>" <?php echo ($_slider[0]->type=='video'?'style="width:57px"':''); ?> />
												<span class="qchero_slide_image_remove" data-slide-id="<?php echo $rows->id; ?>" title="Remove image">X</span>
											<?php else: ?>
												<button class="qchero_slide_image_upload" data-slide-id="<?php echo $rows->id; ?>"><?php echo ($_slider[0]->type=='video'?'Upload Video':'Upload Image'); ?></button>
											<?php endif; ?>
											</div>
										</div>
										<?php if($_slider[0]->type=='video'): ?>
										<div class="qcheroitem-image-cover" style="margin-top: 64px;">
											<div class="cover_image_container">
											<?php if(isset($rows->cover) and $rows->cover!=''): ?>
												<img data-slide-id="<?php echo $rows->id; ?>" src="<?php echo esc_url_raw($rows->cover); ?>"  />
												<span class="qchero_cover_image_remove" data-slide-id="<?php echo $rows->id; ?>" title="Remove image">X</span>
											<?php else: ?>
												<button class="qchero_cover_image_upload" data-slide-id="<?php echo $rows->id; ?>">Upload Cover Image</button>
											<?php endif; ?>
											</div>
										</div>
										<?php endif; ?>
										...
										<div class="qcheroitem-image-cover">
											<div class="cover_image_container">
											<?php if(isset($rows->cover) and $rows->cover!=''): ?>
												<img data-slide-id="<?php echo $rows->id; ?>" src="<?php echo esc_url_raw($rows->cover); ?>"  />
												<span class="qchero_cover_image_remove" data-slide-id="<?php echo $rows->id; ?>" title="Remove image">X</span>
                                        ...
                                        <form class="qchero-nodisplay">
                                            ...
                                            <input type="text" class="qcheroitem-edit-title"
                                                   value="<?php echo wp_unslash( $rows->title ); ?>" placeholder="Default Title">
											...
											<input type="hidden" class="qcheroitem-cover-image-url" value="<?php echo esc_attr($rows->cover); ?>">
											<input type="<?php echo($_slider[0]->type=='no_effect'?'text':'hidden'); ?>" class="qchero_slide_link" value="<?php echo esc_attr($rows->slide_link); ?>" placeholder="URL for linking entire slide">
                                        </form>
										</div>
                                </li>
                                <?php
		...
		<?php endif; ?>
			...
			<?php if($_slider[0]->type=='intro'): ?>
				<h2>TOTAL SLIDE TIME : <span class="total_delay_time"><?php echo $delay_time; ?></span></h2>
			<?php else: ?>
				<h2>Change Effect</h2>
			<?php endif; ?>
				<div class="right_form_effect">
					<form action="admin.php" method="post">
						<select name="effect" id="effect">
							<option value="no_effect" <?php echo ($_slider[0]->type=='no_effect'?'selected="selected"':''); ?>>No Effect</option>
							<option value="video" <?php echo ($_slider[0]->type=='video'?'selected="selected"':''); ?>>Custom Video Slider</option>
							<option value="youtube_video" <?php echo ($_slider[0]->type=='youtube_video'?'selected="selected"':''); ?>>Youtube  Video Slider</option>
							<option value="vimeo_video" <?php echo ($_slider[0]->type=='vimeo_video'?'selected="selected"':''); ?>>Vimeo Video Slider</option>
							<option value="intro" <?php echo ($_slider[0]->type=='intro'?'selected="selected"':''); ?>>Intro Builder</option>
							<option value="aeronautics" <?php echo ($_slider[0]->type=='aeronautics'?'selected="selected"':''); ?>>Aeronautics Effect</option>
							
							<option value="antigravity" <?php echo ($_slider[0]->type=='antigravity'?'selected="selected"':''); ?>>Antigravity Effect</option>
							
							<option value="animated_cloud" <?php echo ($_slider[0]->type=='animated_cloud'?'selected="selected"':''); ?>>Animated Cloud</option>
							
							<option value="atom" <?php echo ($_slider[0]->type=='atom'?'selected="selected"':''); ?>>Atom Effect</option>
							
							<option value="ballsgravity" <?php echo ($_slider[0]->type=='ballsgravity'?'selected="selected"':''); ?>>Balls & Gravity Effect</option>
							<option value="bird" <?php echo ($_slider[0]->type=='bird'?'selected="selected"':''); ?>>Bird Flying Effect</option>
							<option value="blade" <?php echo ($_slider[0]->type=='blade'?'selected="selected"':''); ?>>Blade Effect</option>
							
							<option value="blob" <?php echo ($_slider[0]->type=='blob'?'selected="selected"':''); ?>>Blob Effect</option>
							
							<option value="blur" <?php echo ($_slider[0]->type=='blur'?'selected="selected"':''); ?>>Blur Effect</option>
							
							
							<option value="campfire" <?php echo ($_slider[0]->type=='campfire'?'selected="selected"':''); ?>>Campfire</option>
							
							<option value="circle" <?php echo ($_slider[0]->type=='circle'?'selected="selected"':''); ?>>Circle Effect</option>
							
							<option value="confetti" <?php echo ($_slider[0]->type=='confetti'?'selected="selected"':''); ?>>Confetti Effect</option>
							<option value="cloudysky" <?php echo ($_slider[0]->type=='cloudysky'?'selected="selected"':''); ?>>Cloudy Sky Effect</option>
							<option value="cosmic_web" <?php echo ($_slider[0]->type=='cosmic_web'?'selected="selected"':''); ?>>Cosmic Web Effect</option>
							<option value="colorful_particle" <?php echo ($_slider[0]->type=='colorful_particle'?'selected="selected"':''); ?>>Colorful Particle</option>
							<option value="cursorandpaint" <?php echo ($_slider[0]->type=='cursorandpaint'?'selected="selected"':''); ?>>Cursor And Paint Effect</option>

							<option value="daynight" <?php echo ($_slider[0]->type=='daynight'?'selected="selected"':''); ?>>Day Night Effect</option>
							<option value="division" <?php echo ($_slider[0]->type=='division'?'selected="selected"':''); ?>>Division Effect</option>
							<option value="directional" <?php echo ($_slider[0]->type=='directional'?'selected="selected"':''); ?>>Directional Effect</option>
							<option value="distance" <?php echo ($_slider[0]->type=='distance'?'selected="selected"':''); ?>>Distance Effect</option>
							
							
							<option value="electric_clock" <?php echo ($_slider[0]->type=='electric_clock'?'selected="selected"':''); ?>>Electric Clock</option>
							
							
							<option value="flowingcircle" <?php echo ($_slider[0]->type=='flowingcircle'?'selected="selected"':''); ?>>Flowing Circle Effect</option>
							
							<option value="fizzy_sparks" <?php echo ($_slider[0]->type=='fizzy_sparks'?'selected="selected"':''); ?>>Fizzy Sparks</option>
							
							<option value="firework" <?php echo ($_slider[0]->type=='firework'?'selected="selected"':''); ?>>Firework</option>
							
							<option value="floatrain" <?php echo ($_slider[0]->type=='floatrain'?'selected="selected"':''); ?>>Float Rain Effect</option>
							<option value="floatingleafs" <?php echo ($_slider[0]->type=='floatingleafs'?'selected="selected"':''); ?>>Floating Leafs Effect</option>
							<option value="flyingrocket" <?php echo ($_slider[0]->type=='flyingrocket'?'selected="selected"':''); ?>>Flying Rocket Effect</option>
							
							<option value="grid" <?php echo ($_slider[0]->type=='grid'?'selected="selected"':''); ?>>Grid Effect</option>

							
							
							<option value="chaos" <?php echo ($_slider[0]->type=='chaos'?'selected="selected"':''); ?>>Helix Chaos</option>
							
							<option value="corruption" <?php echo ($_slider[0]->type=='corruption'?'selected="selected"':''); ?>>Helix Corruption Effect</option>
							
							<option value="helix_multiple" <?php echo ($_slider[0]->type=='helix_multiple'?'selected="selected"':''); ?>>Helix Multiple Effect</option>
							
							<option value="hero_404" <?php echo ($_slider[0]->type=='hero_404'?'selected="selected"':''); ?>>Glitch</option>
							
							<option value="iconsahedron" <?php echo ($_slider[0]->type=='iconsahedron'?'selected="selected"':''); ?>>Iconsahedron Effect</option>
							
							
							
							<option value="just_cloud" <?php echo ($_slider[0]->type=='just_cloud'?'selected="selected"':''); ?>>Just Cloud</option>
							
							<option value="line" <?php echo ($_slider[0]->type=='line'?'selected="selected"':''); ?>>Line Effect</option>
							
							<option value="liquid_landscape" <?php echo ($_slider[0]->type=='liquid_landscape'?'selected="selected"':''); ?>>Liquid Landscape</option>
							
							<option value="link_particle" <?php echo ($_slider[0]->type=='link_particle'?'selected="selected"':''); ?>>Link Particle</option>
							
							<option value="matrix" <?php echo ($_slider[0]->type=='matrix'?'selected="selected"':''); ?>>Matrix Effect</option>
							<option value="metaballs" <?php echo ($_slider[0]->type=='metaballs'?'selected="selected"':''); ?>>Metaballs Effect</option>
							<option value="microcosm" <?php echo ($_slider[0]->type=='microcosm'?'selected="selected"':''); ?>>Microcosm Effect</option>
							<option value="svg_animation" <?php echo ($_slider[0]->type=='svg_animation'?'selected="selected"':''); ?>>Moving Color Wave</option>
							
							<option value="nyan_cat" <?php echo ($_slider[0]->type=='nyan_cat'?'selected="selected"':''); ?>>Nyan Cat Effect</option>
							<option value="neno_hexagon" <?php echo ($_slider[0]->type=='neno_hexagon'?'selected="selected"':''); ?>>Neno Hexagon Effect</option>
							<option value="noise_effect" <?php echo ($_slider[0]->type=='noise_effect'?'selected="selected"':''); ?>>Noise Effect</option>
							
							<option value="orbital" <?php echo ($_slider[0]->type=='orbital'?'selected="selected"':''); ?>>Orbital Effect</option>
							
							<option value="particle" <?php echo ($_slider[0]->type=='particle'?'selected="selected"':''); ?>>Particle Effect</option>
							<option value="particle_system" <?php echo ($_slider[0]->type=='particle_system'?'selected="selected"':''); ?>>Particle System</option>
							
							<option value="particle_nasa" <?php echo ($_slider[0]->type=='particle_nasa'?'selected="selected"':''); ?>>Particle Nasa Effect</option>
							<option value="particle_snow" <?php echo ($_slider[0]->type=='particle_snow'?'selected="selected"':''); ?>>Particle Snow Effect</option>
							<option value="particle_bubble" <?php echo ($_slider[0]->type=='particle_bubble'?'selected="selected"':''); ?>>Particle bubble Effect</option>
							<option value="helix" <?php echo ($_slider[0]->type=='helix'?'selected="selected"':''); ?>>Particle Helix Effect</option>
							<option value="physics_bug" <?php echo ($_slider[0]->type=='physics_bug'?'selected="selected"':''); ?>>Physics Bug Effect</option>
							
							<option value="play_or_work" <?php echo ($_slider[0]->type=='play_or_work'?'selected="selected"':''); ?>>Play or Work?</option>
							
							<option value="pretend_hacker" <?php echo ($_slider[0]->type=='pretend_hacker'?'selected="selected"':''); ?>>Hacker</option>
							
							
							<option value="racing_particles" <?php echo ($_slider[0]->type=='racing_particles'?'selected="selected"':''); ?>>Racing Particles</option>
							
							<option value="rain" <?php echo ($_slider[0]->type=='rain'?'selected="selected"':''); ?>>Rain Effect</option>
							<option value="rainy_season" <?php echo ($_slider[0]->type=='rainy_season'?'selected="selected"':''); ?>>Rainy Season</option>
							
							<option value="rays_particles" <?php echo ($_slider[0]->type=='rays_particles'?'selected="selected"':''); ?>>Rays and Particles</option>
							
							<option value="rainofline" <?php echo ($_slider[0]->type=='rainofline'?'selected="selected"':''); ?>>Rain of Line Effect</option>
							
							<option value="rising_cubes" <?php echo ($_slider[0]->type=='rising_cubes'?'selected="selected"':''); ?>>Rising Cubes</option>
							
							<option value="ripples" <?php echo ($_slider[0]->type=='ripples'?'selected="selected"':''); ?>>Ripples Effect</option>
							
							<option value="stars" <?php echo ($_slider[0]->type=='stars'?'selected="selected"':''); ?>>Stars Effect</option>
							
							
							<option value="stellar" <?php echo ($_slider[0]->type=='stellar'?'selected="selected"':''); ?>>Stellar Effect</option>
							
							<option value="stripe-cube" <?php echo ($_slider[0]->type=='stripe-cube'?'selected="selected"':''); ?>>Stripe Cube Effect</option>
							
							<option value="shapeanimation" <?php echo ($_slider[0]->type=='shapeanimation'?'selected="selected"':''); ?>>Shape Animation Effect</option>
							<option value="space_elevator" <?php echo ($_slider[0]->type=='space_elevator'?'selected="selected"':''); ?>>Space Elevator</option>
							
							<option value="squidematics" <?php echo ($_slider[0]->type=='squidematics'?'selected="selected"':''); ?>>Squidematics Effect</option>
							<option value="subvisual" <?php echo ($_slider[0]->type=='subvisual'?'selected="selected"':''); ?>>Subvisual Effect</option>
							
							
							
							<option value="tagcanvas" <?php echo ($_slider[0]->type=='tagcanvas'?'selected="selected"':''); ?>>Tag Canvas Effect</option>
							
							<option value="the_great_attractor" <?php echo ($_slider[0]->type=='the_great_attractor'?'selected="selected"':''); ?>>The Great Attractor</option>
							
							<option value="tiny_galaxy" <?php echo ($_slider[0]->type=='tiny_galaxy'?'selected="selected"':''); ?>>Tiny Galaxy Effect</option>
							<option value="thibaut" <?php echo ($_slider[0]->type=='thibaut'?'selected="selected"':''); ?>>Thibaut Effect</option>
							<option value="cube_animation" <?php echo ($_slider[0]->type=='cube_animation'?'selected="selected"':''); ?>>Cubes Animation</option>
							<option value="valentine" <?php echo ($_slider[0]->type=='valentine'?'selected="selected"':''); ?>>Valentine Effect</option>
							
							<option value="water" <?php echo ($_slider[0]->type=='water'?'selected="selected"':''); ?>>Water Effect</option>
							<option value="wave" <?php echo ($_slider[0]->type=='wave'?'selected="selected"':''); ?>>Wave Effect</option>
							
							<option value="wave_animation" <?php echo ($_slider[0]->type=='wave_animation'?'selected="selected"':''); ?>>Wave Animation Effect</option>
							
							<option value="waaave" <?php echo ($_slider[0]->type=='waaave'?'selected="selected"':''); ?>>Waaave Effect</option>
							<option value="warp_speed" <?php echo ($_slider[0]->type=='warp_speed'?'selected="selected"':''); ?>>Warp Speed Effect</option>
							<option value="walkingbackground" <?php echo ($_slider[0]->type=='walkingbackground'?'selected="selected"':''); ?>>Walking Background Effect</option>
							
							<option value="waving_cloth" <?php echo ($_slider[0]->type=='waving_cloth'?'selected="selected"':''); ?>>Waving Cloth</option>
							
							<option value="waterdroplet" <?php echo ($_slider[0]->type=='waterdroplet'?'selected="selected"':''); ?>>Water Droplet</option>
							<option value="water_swimming" <?php echo ($_slider[0]->type=='water_swimming'?'selected="selected"':''); ?>>Water Swimming Effect</option>
							<option value="wordcloud" <?php echo ($_slider[0]->type=='wordcloud'?'selected="selected"':''); ?>>Word Cloud Effect</option>
							<option value="wormhole" <?php echo ($_slider[0]->type=='wormhole'?'selected="selected"':''); ?>>Wormhole</option>
							
							<option value="ygekpg" <?php echo ($_slider[0]->type=='ygekpg'?'selected="selected"':''); ?>>Ygekpg Effect</option>
...
<a class="<?php echo ($_slider[0]->type=='directional' || $_slider[0]->type=='ygekpg' ?'qchero_preview_p5':'qchero_preview') ?>" href="#" data-id="<?php echo $_id; ?>" style="margin-right: 12px;">Save & Preview</a>		
			</div>
            <div class="settings">
...
<span style="    color: #444;
    ...
    margin-left: 20px;padding: 10px 0px;">Auto:</span> <input class="myElements" name="style[fullwidth]" style="display: inline-block;width: 13px;height: 16px;margin: 0px;float: none;" type="radio" value="3" <?php 
							...
							<div class="othersetting">
								...
								<div style="clear:both"></div>
								<div class="params deseffect customitemstyle">
                                    <label class="customlevel" for="qcslide-effect-slideffect"><?php _e('Slider End Redirect Url', 'qcslide'); ?></label>
									<input class="myElements" style="width: 96%;" type="text" name="params[slidendredirect]"
											   value="<?php echo (isset($params->slidendredirect) && $params->slidendredirect!=''?$params->slidendredirect:''); ?>" placeholder="Ex: http://www.example.com" />
                                </div>
								<?php if($_slider[0]->type=='intro'): ?>
								<div class="params deseffect customitemstyle">
									<div style="display: inline-block;">Or</div>
									<div style="display: inline-block;width: 92%;">
										<label class="customlevel" for="qcslide-effect-slideffect"><?php _e('Slider End Load New slider', 'qcslide'); ?></label>
											...
											<?php
												...
												foreach($row as $rows){
													if(isset($params->newsliderafterend) and $params->newsliderafterend==$rows->id){
														...
														echo '<option value="'.esc_attr($rows->id).'" selected="selected">'.esc_html($rows->title).'</option>';
								...
								<div class="params customitemstyle" >
									<label class="customlevel" for="qchero-caption-text-color" style="display: inline-block;">Show Skip Button</label> 
									<select name="params[heroskip]">
										<option value="1" <?php echo (isset($params->heroskip)&& $params->heroskip==1?'selected="selected"':''); ?>>Hide</option>
										<option value="0" <?php echo (isset($params->heroskip)&& $params->heroskip==0?'selected="selected"':''); ?>>Show</option>
										
									</select>
								</div>
								<?php endif; ?>
								<div class="params deseffect customitemstyle">
                                    <label class="customlevel" for="qcslide-effect-slideffect"><?php _e('Slider Redirect Delay', 'qcslide'); ?></label>
									<input class="myElements" style="width: 96%;" type="number" name="params[slideredirectdelay]"
											   value="<?php echo esc_attr(isset($params->slideredirectdelay)?$params->slideredirectdelay:''); ?>" placeholder="Ex: 1000" />
								...
								<div class="params customitemstyle" >
									<label class="customlevel" for="qchero-caption-text-color" style="display: inline-block;">Reverse the Slide in Front End</label> 
									<select name="params[reverse_slide]">
										<option value="0" <?php echo (isset($params->reverse_slide)&& $params->reverse_slide==0?'selected="selected"':''); ?>>Off</option>
										<option value="1" <?php echo (isset($params->reverse_slide)&& $params->reverse_slide==1?'selected="selected"':''); ?>>On</option>
									</select>
								</div>
								
								<div class="params customitemstyle" >
									<label class="customlevel" for="qchero-caption-text-color" style="display: inline-block;">Slide Start after Certain MS</label> 
									<input class="myElements" style="width: 96%;" type="text" name="params[first_slide_delay]"
											   value="<?php echo (isset($params->first_slide_delay) && $params->first_slide_delay!=''?$params->first_slide_delay:'0'); ?>" placeholder="1000" />
								</div>
								
								<div class="params customitemstyle" style="width: 37%;" >
									<label class="customlevel" for="qchero-caption-text-color" style="display: inline-block;">Display the Title, Description, Buttons at the bottom</label> 
									<select name="params[content_display_bottom]">
										<option value="0" <?php echo (isset($params->content_display_bottom)&& $params->content_display_bottom==0?'selected="selected"':''); ?>>Off</option>
										<option value="1" <?php echo (isset($params->content_display_bottom)&& $params->content_display_bottom==1?'selected="selected"':''); ?>>Desktop & Mobile</option>
										<option value="2" <?php echo (isset($params->content_display_bottom)&& $params->content_display_bottom==2?'selected="selected"':''); ?>>Mobile Only</option>
									</select>
								</div>
								<?php endif; ?>
						...
						<?php endif; ?>
							...
							<div class="params customitemstyle">
								...
								<!--<div class="params customitemstyle">
									<label class="customlevel" for="qchero-caption-text-color">Desctiption bg Color</label> <input type="text" name="params[descriptionbgcolor]" class="color-field myElements" value="<?php echo esc_attr(isset($params->descriptionbgcolor)?esc_attr($params->descriptionbgcolor):''); ?>" />
								</div>--> 
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								...
								<div class="params customitemstyle" style="width:100%;">
									...
									<label class="customlevel" for="bg_video">Background Image Effect</label>
									<select class="myElements" name="params[bgimageeffect]">
									
										<option value="">None</option>
										<option value="parallax" <?php echo (isset($params->bgimageeffect)&&$params->bgimageeffect=='parallax'?'selected="selected"':''); ?>>Parallax Effect</option>
										<option value="ken-burn" <?php echo (isset($params->bgimageeffect)&&$params->bgimageeffect=='ken-burn'?'selected="selected"':''); ?>>Ken Burn Effect</option>
...
<?php 
$sarray = array('just_cloud','matrix','neno_hexagon','play_or_work','rain','rising_cubes','stars','torus','warp_speed','wormhole','blade','blur','daynight','division','fizzy_sparks','firework','rainy_season');
if(in_array($_slider[0]->type,$sarray)):
?>
					<div style="clear:both"></div>
					<p style="color:red;font-size: 15px;">This effect does not support Background Color, Gradient or Image.</p>
<?php endif; ?>	
...
?>
						...
						<li class="video-settings">
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									...
									<select name="params[video]" id="sh_video" class="myElements">
										<option value="">None</option>
										<option value="custom" <?php echo (isset($params->video)&& $params->video=='custom'?'selected="selected"':''); ?>>Custom Video</option>
										<option value="youtube" <?php echo (isset($params->video)&& $params->video=='youtube'?'selected="selected"':''); ?>>Youtube Video</option>
										<option value="vimeo" <?php echo (isset($params->video)&& $params->video=='vimeo'?'selected="selected"':''); ?>>Vimeo Video</option>
									</select>
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;" id="">
								<div class="params customitemstyle" style="width:100%;">
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									...
									<label class="customlevel" for="bg_image_url">Cover Image</label>
									<input type="hidden" name="params[cover_image_url]" id="cover_image_url" class="regular-text" value="<?php echo esc_attr(isset($params->cover_image_url)?$params->cover_image_url:''); ?>">
									<input type="button" name="upload-btn" id="cover-upload-btn" class="button-secondary" value="Upload Image">
								</div>
								<div style="clear:both"></div>
								<div id="bg_preview_img" class="cover_preview_image">
								
								<?php 
									if(isset($params->cover_image_url) && $params->cover_image_url != '') :
								?>
									<span class="remove_bg_image remove_cover_image">X</span>
									<img src="<?php echo esc_attr($params->cover_image_url); ?>" alt="" />
								<?php endif; ?>
								</div>
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									<label class="customlevel" for="videoslide_mute">Show Cover Image First, Play on Click</label>
									<select name="params[show_cover_image]" id="show_cover_image" class="myElements">
										<option value="0" <?php echo (isset($params->show_cover_image)&& $params->show_cover_image=='0'?'selected="selected"':''); ?>>False</option>
										<option value="1" <?php echo (isset($params->show_cover_image)&& $params->show_cover_image=='1'?'selected="selected"':''); ?>>True</option>
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									...
									<label class="customlevel" for="bg_image_url">Global Cover Image</label>
									<input type="hidden" name="params[cover_image_url]" id="cover_image_url" class="regular-text" value="<?php echo esc_attr($params->cover_image_url); ?>">
									<input type="button" name="upload-btn" id="cover-upload-btn" class="button-secondary" value="Upload Image">
								</div>
								<div style="clear:both"></div>
								<div id="bg_preview_img" class="cover_preview_image">
							...
							<h2>Video Popup & Play Button Position Settings</h2>
							<hr>
							
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									
									<label class="customlevel" for="videoslide_mute">Show Video On Popup</label>
									<select name="params[show_video_popup]" id="show_video_popup" class="myElements">
										<option value="0" <?php echo (isset($params->show_video_popup)&& $params->show_video_popup=='0'?'selected="selected"':''); ?>>False</option>
										<option value="1" <?php echo (isset($params->show_video_popup)&& $params->show_video_popup=='1'?'selected="selected"':''); ?>>True</option>
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									<label class="customlevel" for="play_button_position_horizontal">Play Button Position Horizontal</label>
									<select name="params[play_button_position_horizontal]" id="play_button_position_horizontal" class="myElements">
										<option value="center" <?php echo (isset($params->play_button_position_horizontal)&& $params->play_button_position_horizontal=='center'?'selected="selected"':''); ?>>Center</option>
										<option value="left" <?php echo (isset($params->play_button_position_horizontal)&& $params->play_button_position_horizontal=='left'?'selected="selected"':''); ?>>Left</option>
										<option value="right" <?php echo (isset($params->play_button_position_horizontal)&& $params->play_button_position_horizontal=='right'?'selected="selected"':''); ?>>Right</option>
									</select>
								</div>
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%;">
									<label class="customlevel" for="play_button_position_vertical">Play Button Position Vertical</label>
									<select name="params[play_button_position_vertical]" id="play_button_position_vertical" class="myElements">
										<option value="center" <?php echo (isset($params->play_button_position_vertical)&& $params->play_button_position_vertical=='center'?'selected="selected"':''); ?>>Center</option>
										<option value="top" <?php echo (isset($params->play_button_position_vertical)&& $params->play_button_position_vertical=='top'?'selected="selected"':''); ?>>Top</option>
										
										<option value="bottom" <?php echo (isset($params->play_button_position_vertical)&& $params->play_button_position_vertical=='bottom'?'selected="selected"':''); ?>>Bottom</option>
									</select>
								</div>
							</div>
						...
						<li class="audio-setting">
							...
							<div class="othersetting" style="display: inline-block; float: left; width: 25%;">
								<div class="params customitemstyle" style="width:100%">
									<label class="customlevel" for="qchero-background-color" style="display: inline-block;">Audio Repeat</label> 
									<select name="params[audiorepeat]">
										<option value="1" <?php echo (isset($params->audiorepeat)&& $params->audiorepeat==1?'selected="selected"':''); ?>>True</option>
						...
						<li class="Effect-setting">
							...
							<?php 
								elseif(isset($_slider[0]->type) and $_slider[0]->type=='flowingcircle') :
							?>
							
								<div class="params customitemstyle">
									<label class="customlevel" for="qchero-background-color">Animation Repeat</label> <select name="params[flowingcircle_animation]">
											<option value="0" <?php echo (isset($params->flowingcircle_animation) &&$params->flowingcircle_animation=='0'?'selected="selected"':''); ?>>False</option>
											<option value="1" <?php echo (isset($params->flowingcircle_animation) && $params->flowingcircle_animation=='1'?'selected="selected"':''); ?>>True</option>
									</select>
								</div>
								
								<div class="params customitemstyle">
									<label class="customlevel" for="qchero-caption-text-color">Animation Repeat Interval</label> <input type="text" name="params[flowingcircle_speed]" class="myElements" value="<?php echo (isset($params->flowingcircle_speed)?esc_attr($params->flowingcircle_speed):''); ?>" placeholder="Ex: 4000" />
									<p>1000 for 1sec.</p>
								</div>
...
<div id="qchero_loading_overlay" style="display:none;">
    ...
    <style>
        ...
        #qchero-title-construct {
            ...
            opacity: 0.9;
            color: rgb(86, 88, 85);
            filter: alpha(opacity=<?php echo abs(isset($params->title->style->opacity)?$params->title->style->opacity:0);?>);
