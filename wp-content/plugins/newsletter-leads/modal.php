...
<div id="tnp-modal-html" class="tnp-modal">
    ...
    <div class="tnp-popup-main">
        <form action="#" method="post" onsubmit="tnp_leads_submit(this); return false;">
            ...
            <?php if (!empty($options['theme_field_gender'])) { ?>
                <div class="tnp-field tnp-field-gender">
                    <?php if (!$placeholders) { ?>
                        <label><?php echo esc_html($subscription->get_form_text('sex')) ?></label>
                    <?php } ?>

                    <select name="nx" class="tnp-gender" id="tnp-gender" required>
                        <option value="n"><?php echo esc_html($subscription->get_form_text('sex_none')); ?></option>
                        <option value="f"><?php echo esc_html($subscription->get_form_text('sex_female')); ?></option>
                        <option value="m"><?php echo esc_html($subscription->get_form_text('sex_male')); ?></option>
                    </select>
                </div>
            <?php } ?>
            ...
            <?php
            if (isset($options['customfields']) && is_array($options['customfields'])) {

                $field_attrs = [];
                $field_attrs['name'] = 'customfield';
                foreach ($options['customfields'] as $id) {
                    $profile = $subscription->get_customfield($id);
                    ...
                    if ($placeholders) {
                        $field_attrs['label'] = '';
                        $field_attrs['placeholder'] = $profile->name;
                    }
                    $field_attrs['number'] = $id;
                    echo $subscription->shortcode_newsletter_field($field_attrs);
                }
            }
            ?>
