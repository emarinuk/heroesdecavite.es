...
<div class="wrap" id="tnp-wrap">
    ...
    <div id="tnp-body">
        ...
        <?php
        $urls = $wpdb->get_results("select url, count(distinct user_id) as number from " . NEWSLETTER_STATS_TABLE . " where url<>'' and url not like '%na=%nk=%' and email_id=" . ( (int) $email->id ) . " group by url order by number desc");
        $total = $wpdb->get_var("select count(distinct user_id) from " . NEWSLETTER_STATS_TABLE . " where url<>'' and url not like '%na=%nk=%' and email_id=" . ( (int) $email->id ));
        ?>
        ...
        <?php else: ?>
            <table class="widefat">
                ...
                <thead>
                    <tr class="text-left">
                        <th>Link</th>
                        <th>Subscribers <i class="fas fa-info-circle" title="Subscribers who clicked the link"></i></th>
                        <th>%</th>
                        <th>For who clicked...</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <?php for ($i = 0; $i < count($urls); $i++) : ?>
                        <tr>
                            ...
                            <td>
                                <?php echo esc_html($this->percent($urls[$i]->number, $total)); ?>
                            </td>
                            <td>
